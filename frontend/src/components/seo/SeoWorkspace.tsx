...
type Tab = "overview" | "settings" | "aeo" | Module
export let SeoWorkspace = ({ siteUrl, children }: { siteUrl: string; children: ReactNode }) => {
 ...
 useEffect(() => { setSite(null); setTab("overview"); if (admin) void refresh() }, [admin, refresh])
 ...
 return <div className={styles.workspace}>
  <nav className={styles.tabs} aria-label="Website reports">
   {(["overview", ...Object.keys(LABELS), "aeo", "settings"] as Tab[]).map(key => <TabButton key={key} value={key} selected={tab} select={setTab} label={key === "overview" ? "Overview" : key === "settings" ? "SEO settings" : key === "aeo" ? "AI Visibility" : LABELS[key]} />)}
  </nav>
  {error && <p role="alert" className="error-text">{error}</p>}
  {tab === "overview" && children}
  {tab === "aeo" && <AeoSection siteUrl={siteUrl} />}
  {tab !== "overview" && tab !== "aeo" && !site && !error && <p>Loading SEO settings…</p>}
  {site && tab === "settings" && <SettingsForm initial={site.config} save={save} busy={busy} />}
  {site && tab in LABELS && <ModuleView key={tab} module={tab as Module} site={site} run={run} cancel={cancel} busy={busy} save={save} />}
 </div>
}
let TabButton = ({ value, selected, select, label }: { value: Tab; selected: Tab; select: (tab: Tab) => void; label: string }) => {
 let click = () => select(value)
 return <button type="button" aria-current={value === selected ? "page" : undefined} onClick={click}>{label}</button>
}
...
let ModuleView = ({ module, site, run, cancel, busy, save }: { module: Module; site: Site; run: (module: Module) => Promise<void>; cancel: (id: string) => Promise<void>; busy: boolean; save: (config: Settings) => Promise<void> }) => {
 ...
 if (!site.config.modules[module].enabled) return <section className={styles.empty}><h2>{LABELS[module]}</h2><p>This module is disabled for this website. Enable it in SEO settings to start collecting data.</p></section>
 return <section className={styles.module}>
  <div className={styles.actions}><h2>{LABELS[module]}</h2><button type="button" disabled={busy || !!active} onClick={start}>Run now</button>{active && <><span role="status">{active.status === "waiting_browser" ? online ? "Browser research queued or running" : "Waiting for bmux machine" : active.status}</span><button type="button" onClick={stop}>Cancel</button></>}</div>
  ...
  {latest && <Results key={latest.id} job={latest} config={site.config} save={save} />}
 </section>
}
let Results = ({ job, config, save }: { job: Job; config: Settings; save: (config: Settings) => Promise<void> }) => {
 ...
 return <div className={styles.results}>
  ...
  {browser && <p>Desktop · requested country: {text(browser.requested_country)} · language: {text(browser.requested_language)}. Location follows the browser’s network; country selection is a hint.</p>}
  ...
  {job.module === "research" && <><h3>Observed competitors</h3><p>{text(result.competitors)}</p><h3>Link prospects</h3><p>Links below were observed on inspected pages. This is a sample of opportunities, not a complete backlink index.</p><DataTable data={filtered(rows(result.prospects))} columns={[["url", "Page"], ["title", "Title"], ["owned_links", "Links to this site"], ["competitor_links", "Links to competitors"], ["evidence", "Page excerpt"]]} /><h3>Inspected competitor pages</h3><DataTable data={filtered(rows(result.pages))} columns={[["url", "Page"], ["title", "Title"], ["h1", "Headings"], ["description", "Description"]]} /><h3>Keyword ideas</h3><KeywordTable data={filtered(rows(result.suggestions))} config={config} save={save} /></>}
  ...
  {rows(result.snapshots).length > 0 && <details><summary>SERP evidence</summary>{rows(result.snapshots).map(snapshot => <div key={text(snapshot.keyword)}><h3>{text(snapshot.keyword)}</h3><DataTable data={rows(snapshot.entries)} columns={[["position", "Position"], ["title", "Title"], ["url", "URL"], ["snippet", "Observed text"]]} /></div>)}</details>}
  {[...rows(result.errors), ...rows(result.render_errors)].length > 0 && <><h3>Incomplete checks</h3><DataTable data={[...rows(result.errors), ...rows(result.render_errors)]} columns={[["url", "URL"], ["keyword", "Keyword"], ["error", "Reason"]]} /></>}
 </div>
}
